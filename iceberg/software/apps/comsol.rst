...
- The amount of real (``rmem``) and virtual memory (``mem``) that the job can use;
- The maximum amount of virtual memory used by Java (``-Xmx2G`` means that it can use 2GB of virtual memory; this quantity should be significantly smaller than the amount of memory requested using ``-l mem=``);
