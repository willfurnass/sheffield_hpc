...
tar -xvf $filename
cd SU2-$version
# export PYTHON_INCLUDE to address the following error
# configure: error: cannot find python-config for /bin/python.
export PYTHON_INCLUDE=/usr/include/python2.7
./configure --prefix=$prefix --enable-mpi --with-cc=$(which mpicc) --with-cxx=$(which mpicxx)
make
make install
chmod -R g+w /usr/local/packages/apps/su2/$version
