...
With SGE you can robustly submit, control and monitor jobs from programs (e.g C or Python) programs using the :ref:`DRMAA <drmaa>` API/libraries.

Slurm has a `C API <https://slurm.schedmd.com/api.html>`__ and a `REST API <https://slurm.schedmd.com/rest.html>`__ 
(that can programmatically be accessed over HTTP).  Neither of these has been tested at the University of Sheffield.
...
such as the :ref:`SGE <sched>` software used on 
the :ref:`sharc` and :ref:`iceberg` clusters.  

Support for DRMAA is built in to SGE and other scheduling software such as Torque, PBS, HTCondor and Slurm.
