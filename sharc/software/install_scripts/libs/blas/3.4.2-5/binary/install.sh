...
MEDIA_DIR="/usr/local/media/blas/${VERS}"
RPM_FNAME="blas-$VERS.el7.x86_64.rpm"
RPM_DEV_FNAME="blas-devel-$VERS.el7.x86_64.rpm"

RPM_FPATH="${MEDIA_DIR}/${RPM_FNAME}"
RPM_DEV_FPATH="${MEDIA_DIR}/${RPM_DEV_FNAME}"

INSTALL_DIR="/usr/local/packages/libs/blas/${VERS}/binary"
...
if [[ ! -f $RPM_FPATH ]]; then
    echo "Could not find BLAS RPM  at: ${RPM_FPATH}; exiting." 2>&1 
    exit -1
fi

if [[ ! -f $RPM_DEV_FPATH ]]; then
    echo "Could not find BLAS DEV RPM  at: ${RPM_DEV_FPATH}; exiting." 2>&1 
    exit -1
fi
