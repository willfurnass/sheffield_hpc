...
############################## Variable Setup ################################
version=2016.4
prefix=/usr/local/packages/apps/gromacs/$version/gcc-4.9.4
build_dir=/scratch/$USER/gromacs

filename=/usr/local/media/gromacs/gromacs-$version.tar.gz
baseurl=
...
make
make check
# Successful "make check" gives: 100% tests passed, 0 tests failed out of 27
make install
